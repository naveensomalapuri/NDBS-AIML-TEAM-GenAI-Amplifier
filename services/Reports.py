...
# Sub-model for Input Selection Criteria
...
# Sub-model for Security Requirements
class SecurityRequirements(BaseModel):
    ...
    no_specific_restrictions: Optional[str] = Field(None, description="Functional Design - Security Requirements: No Specific Restrictions")
    restriction_based_on_certain_criteria: Optional[str] = Field(
        None, description="Functional Design - Security Requirements: Restriction Based on Certain Criteria (ex: Restricted by Sales area?)"
    )
    other: Optional[str] = Field(None, description="Functional Design - Security Requirements: Other")

# Sub-model for Unit Testing details
class UnitTesting(BaseModel):
    unit_testing: str = Field(..., description="Functional Design - Unit Testing: (Information the developer can use to unit test the application. This needs to include test scenarios, instructions, test data, and expected results)")
    test_condition_test_scenario: Optional[str] = Field(None, description="Functional Design - Test Condition / Test Scenario")
    steps_involved: Optional[str] = Field(None, description="Functional Design - Steps Involved")
    input_values_test_data: Optional[str] = Field(None, description="Functional Design - Input Values (Test Data)")
    expected_results: Optional[str] = Field(None, description="Functional Design - Expected Results")

# Main Functional Design model
class FD(BaseModel):
    Functional_Design_reference_sap_fiori_tiles_transactions_tables_programs: str = Field(
        ..., description="Functional Design - Reference SAP Fiori Tiles/Transactions/Tables/Programs: (Include all known transactions, tables and programs that are used, or reports containing similar data. Screenshots are recommended.)"
    )
    ...
    Functional_Design_output_report_layout: str = Field(
        ..., description="Functional Design - Output/Report Layout: (Description of the layout and screen information, columns, headings, summations, etc.)"
    )
    Functional_Design_column_title_as_it_should_appear_on_the_report_output: Optional[str] = Field(
        None, description="Functional Design - Column Title (as it should appear on the report output)"
    )
    Functional_Design_reference_table_field_output: Optional[str] = Field(
        None, description="Functional Design - Reference Table-Field for Output"
    )
    Functional_Design_sort: Optional[str] = Field(None, description="Functional Design - Sort")
    Functional_Design_group_level: Optional[str] = Field(None, description="Functional Design - Group Level")
    Functional_Design_hotspot: Optional[str] = Field(None, description="Functional Design - Hotspot")
    Functional_Design_drill_down: Optional[str] = Field(None, description="Functional Design - Drill Down")
    Functional_Design_column_description: Optional[str] = Field(None, description="Functional Design - Description of the Column")
    Functional_Design_special_functionality: str = Field(
        ..., description="Functional Design - Special Functionality: (List any additional functionality required, such as drill-down/hotspots, links to other transactions, etc.)"
    )
    Functional_Design_error_handling: str = Field(
    ...
    Functional_Design_security_requirements: Optional[SecurityRequirements] = Field(
        None, description="Functional Design - Security Requirements"
    )
    ...
    Functional_Design_unit_testing: UnitTesting = Field(
        ..., description="Functional Design - Unit Testing Section"
    )
    Functional_Design_additional_comments: str = Field(
